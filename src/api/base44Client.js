...
const enabled = import.meta.env.VITE_USE_BASE44 === 'true';
const mockClient = {
  entities: { /* 네가 가진 mock 그대로 */ },
  auth: {
    login: async () => ({ user: { id: 'mock-user' } }),
    logout: async () => {},
    getCurrentUser: async () => ({ id: 'mock-user', name: 'Mock User' }),
    me: async () => ({ id: 'mock-user', name: 'Mock User', email: 'mock@example.com' }),
  },
  integrations: { /* …생략… */ },
};

export const base44 = enabled
  ? createClient({
      // 여긴 나중에 활성화할 때만 채움
      appId: "68ad8266d3f7a3e6710e5720",
      requiresAuth: true,
      // redirectUri: import.meta.env.VITE_BASE44_REDIRECT_URI,  // 도메인 준비되면 추가
      // issuer/clientId 등 필요값도 같이
    })
  : mockClient;